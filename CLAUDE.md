...
- Build: `node build.mjs` (wynik w `dist/`; generuje też 59 statycznych stron `/product/<id>/index.html` - pełny shell aplikacji z podmienionymi meta i prerenderem treści przez współdzielony `js/seo-render.js` - oraz `sitemap.xml`, `robots.txt`, `_redirects`, `404.html` i `.nojekyll`). Zmienne: `BASE_PATH` (podkatalog hostingu, np. `/ramnevercomes` na GitHub Pages; domyślnie pusty) i `SITE_ORIGIN` (publiczny adres do canonical/og/sitemap; domyślnie `https://ramnevercomes.com`). Pod prefiksem linki `href="/..."` dostają go w buildzie, a router (`BASE_PATH` z `<base href>`, `stripBase`) trzyma trasy względem aplikacji. `./stage.sh <dir>` składa katalog publikacji (dist + assets + klucz IndexNow)
...
- Deploy: produkcja docelowo na GitHub Pages (`https://stmn.github.io/ramnevercomes`, workflow `.github/workflows/pages.yml` przy pushu na `main`; domena ramnevercomes.com to redirect 301 do wygaśnięcia). `./deploy.sh` (Cloudflare Pages, projekt `ramnevercomes`) zostaje jako droga awaryjna - TYLKO na jawne polecenie
